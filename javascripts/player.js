...
class Player {
  ...
  moveX(time, state, keys, overlap) {
    this.speed.x = 0;
    if (keys.left && this === state.player && !overlap.includes('rightOverlap'))
      this.speed.x -= this.xSpeed;
    if (keys.right && this === state.player && !overlap.includes('leftOverlap'))
      this.speed.x += this.xSpeed;
    ...
    if (
      (overlap.includes('leftOverlap') && movedX < this.pos.x) ||
  ...
  moveY(time, state, keys, overlap) {
    ...
    if (
      obstacle ||
      overlap.includes('topOverlap') ||
      (overlap.includes('bottomOverlap') &&
        (this === state.player || state.nonPlayers.includes(this)))
    ) {
      if (['gravity', 'poison', 'instruction'].includes(obstacle)) {
        this.pos = newPos;
      } else if (overlap.includes('Platform')) {
        const platform = overlap.find(
          overlapType => typeof overlapType === 'object'
        ).platform;
        ...
        keys.up &&
        ((this.speed.y >= 0 && !(this.speed.y <= 1 && obstacle === 'water')) ||
          overlap.includes('topOverlap')) &&
        this === state.player
      ) {
        this.playJumpAudio(state);
  ...
  update(time, state, keys) {
    ...
    if (state.status !== 'won') {
      if (
        !(overlap.includes('leftOverlap') && overlap.includes('rightOverlap'))
      ) {
        this.moveX(time, state, keys, overlap);
      }
      if (
        !(overlap.includes('topOverlap') && overlap.includes('bottomOverlap'))
      ) {
